...
int sequence(int num[], int length, int (*memo)[length+1], int index, int prev) {
    if(index == length)
        return 0;
    ...
    if(prev == -1 || num[index] > num[prev])
        result1 = 1 + sequence(num, length, memo, index+1, index);
    result2 = sequence(num, length, memo, index+1, prev);
...
int increase(int num[], int length)
{
    ...
    return sequence(num, length, memo, 0, -1);
}
