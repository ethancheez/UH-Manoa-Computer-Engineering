...
k = kth_smallest(a, 0, LENGTH2-1, 6);
...
int kth_smallest(int a[], int first, int last, int k)
{
    ...
    if(pivot_pos - first == k)
        return a[pivot_pos];
    else if(pivot_pos - first > k)
        return kth_smallest(a, first, pivot_pos - 1, k);

    return kth_smallest(a, pivot_pos + 1, last, k - pivot_pos + first - 1);
}
